...
def display_map(map_html_file):
    # Build the path to the map file within the visualizations folder
    file_path = os.path.join('visualizations', map_html_file)
    try:
...
def display_visualization(disaster):
    ...
    for vis in visualization_files.get(disaster, []):
        try:
            file_path = os.path.join('visualizations', f"{vis}.html") # Update with correct path
            with open(file_path, 'r', encoding='utf-8') as f:
                html_content = f.read()
